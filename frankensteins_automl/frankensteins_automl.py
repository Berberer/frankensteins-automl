...
class FrankensteinsAutoML:
    ...
    def _load_data(self):
        arff_data = arff.load(open(self.config.data_path, "r"))
        data = numpy.array(arff_data["data"])
        data = data.astype(numpy.float64)
        target_index = len(arff_data["attributes"]) - 1
        data_x = data[:, :target_index]
        data_y = data[:, target_index]
